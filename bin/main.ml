...
let parse_logic filename = read_file (Parser2.logic_file Lexer2.token) filename

let parse_problem filename =
  read_file (Parser2.problem_file Lexer2.token) filename

let usage_msg =
  "Usage: pgeon [--log-level debug|info|warn|error] <logic-file> \
   <problem-file>"
...
let () =
  let rec parse_args level files = function
    ...
    | "--log-level" :: lvl :: rest -> (
        ...
        | None, None ->
            Error
              (Printf.sprintf "Unknown log level '%s'. Expected one of debug, \
                               info, warn, error."
                 lvl)
